    ...
    <WidgetShell id={widget.id} title={widget.title} icon={<RiStockLine />} x={widget.x} y={widget.y} width={widget.width} height={widget.height} minimized={widget.minimized}>
      <div className="flex h-full flex-col p-4">
        ...
        <div className="mb-3 flex items-center gap-2">
          ...
          <button onClick={addStock} disabled={loading} className="rounded-xl bg-amber-500 px-4 py-2 text-[10px] font-black tracking-[0.18em] text-black disabled:opacity-40">
            {loading ? <RiLoader4Line className="animate-spin" size={14} /> : 'ADD'}
          </button>
          <button onClick={loadDefaults} disabled={refreshing} className="rounded-xl border border-white/10 p-2 text-zinc-400 hover:text-amber-400">
        ...
        <div className="scrollbar-small flex-1 space-y-2 overflow-y-auto">
          {stocks.length === 0 ? (
            ...
            <div key={stock.symbol} className="group flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.02] px-4 py-3 transition-all hover:border-amber-500/20">
              ...
              <div className="text-right">
                ...
                <div className={`flex items-center justify-end gap-1 text-[11px] font-bold ${stock.change >= 0 ? 'text-amber-400' : 'text-red-400'}`}>
